...
void add_identity_to_sparse_jacobian(SMatrix &jac, size_t start_i,
                                     size_t start_j, size_t size,
                                     double identity_coef) {
...
public:
  ...
  void evaluate(const Eigen::VectorXd &x, Eigen::VectorXd &values,
    ...
    for (size_t t = 0; t < T_ - 1; t++) {
      ...
      values.segment(c_head, 2) =
          x_next - (x_now + v_now * dt_ + 0.5 * a_now * dt_ * dt_);
    ...
    for (size_t t = 0; t < T_ - 1; t++) {
      ...
      add_identity_to_sparse_jacobian(jacobian, i_head, j_head + 4, 2,
                                      -0.5 * dt_ * dt_);
      add_identity_to_sparse_jacobian(jacobian, i_head, j_head + 6, 2, 1.0);

      // diff-const for v_t+1 = v_t + a_t * dt
      add_identity_to_sparse_jacobian(jacobian, i_head + 2, j_head + 2, 2,
                                      -1.0);
      add_identity_to_sparse_jacobian(jacobian, i_head + 2, j_head + 4, 2,
                                      -dt_);
...
public:
  EndPointsConstraint(size_t T, const Eigen::Vector2d &start,
                      const Eigen::Vector2d &goal)
      : EqualityConstraintBase(6 * T, "end-point", 1e-2, true), T_(T),
        start_(start), goal_(goal) {}

  size_t get_cdim() override { return 8; }
...
public:
  ...
  CollisionConstraint(size_t T, double dt)
      : InequalityConstraintBase(6 * T, "collision", 1e-4, true), T_(T) {
  ...
  void evaluate(const Eigen::VectorXd &x, Eigen::VectorXd &values,
    ...
    for (size_t t = 0; t < T_; t++) {
      ...
      for (size_t i = 0; i < obstacles_.size(); i++) {
        double dist =
            (x_now - obstacles_[i].center).norm() - obstacles_[i].radius;
        if (dist < min_dist) {
          min_dist = dist;
  ...
  size_t get_cdim() override { return 2 * T_ * obstacles_.size(); }
  size_t T_;
...
TEST(SOLVE_TRAJOPT, SQPTEST) {
  ...
  option.max_iter = 200;
  auto solver = NLPSolver(cstset->nx_, P, Eigen::VectorXd::Zero(cstset->nx_),
                          cstset, option);
