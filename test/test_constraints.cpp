...
TEST(ConstraintSet, SQPTest) {
  ...
  // test jacobian
  Eigen::MatrixXd jacobian_expected =
      Eigen::MatrixXd::Zero(cstset.get_cdim(), 2);
  jacobian_expected << 1.0, 0.0, 0.0, 1.0, 2 * x(0) + x(1), x(0), x(1),
      2 * x(1) + x(0), 1.0 + x(1), x(0), x(1), 1.0 + x(0);
  double max_error =
      (jacobian.toDense() - jacobian_expected).cwiseAbs().maxCoeff();
  EXPECT_LT(max_error, 1e-10);

  // test values
  Eigen::VectorXd values_expected = Eigen::VectorXd::Zero(cstset.get_cdim());
  values_expected << x(0), x(1), x(0) + x(0) * x(1), x(1) + x(0) * x(1),
  ...
  lower_expected << 1., 1., 3 * x(0) + x(1) - (x(0) * x(0) + x(0) * x(1)),
  ...
  upper_expected << 3., 3., 3 * x(0) + x(1) - (x(0) * x(0) + x(0) * x(1)),
      3 * x(1) + x(0) - (x(1) * x(1) + x(0) * x(1)),
